...
HighThresholdRatio=0.135;%Define highThresholdRatio
lowThresholdRatio=0.5;%Define lowThresholdRatio
highThreshold=max(max(Mag))*HighThresholdRatio;%Compute high threshold
lowThreshold=lowThresholdRatio*highThreshold;%Compute low threshold
...
N1=interp2(X,Y,Mag,Xquery1, Yquery1);%Query pixel intensity along edge
...
for i=1:strongIndex
       E = VerifyWeakEdges(E, strongEdgesRow(i),...
             strongEdgesCol(i), N1,N2,Ori,Ori1,lowThreshold);
end
